...
def book_slot():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        if result:
            existing_vehicle_number, existing_slot_number = result
            # If vehicle number is already booked
            if existing_slot_number <= 10:
                return render_template('book_car.html', message="This vehicle number is already booked.", vehicle_number=vehicle_number, vehicle_owner=vehicle_owner, in_time=in_time, out_time=out_time, slot_number=slot_number)
            else:
                return render_template('book_bike.html', message="This vehicle number is already booked.", vehicle_number=vehicle_number, vehicle_owner=vehicle_owner, in_time=in_time, out_time=out_time, slot_number=slot_number)
